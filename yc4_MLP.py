...
class AudioClassifier(nn.Module):
    def __init__(self, num_classes, input_dim):
        ...
        # 定义全连接层
        self.fc1 = nn.Linear(input_dim, 512)  # 第一个全连接层，输入维度为特征维度，输出维度为512
        self.fc2 = nn.Linear(512, 256)        # 第二个全连接层，输出维度为256
        self.fc3 = nn.Linear(256, 128)        # 第三个全连接层，输出维度为128
        self.fc4 = nn.Linear(128, num_classes) # 最后一个全连接层，输出维度为类别数

        self.dropout = nn.Dropout(0.5)  # 引入Dropout防止过拟合，丢弃率为0.5

    def forward(self, x):
        # 前向传播，应用激活函数ReLU
        x = F.relu(self.fc1(x))
        x = self.dropout(x)  # 在第一层和第二层之间应用dropout
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        x = self.fc4(x)  # 最后一层通常不用激活函数，直接输出
        return x
